...
export class GmControlsApplication extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext() {
    ...
    return {
      ...
      players,
      screens: Object.entries(TerminalApplication.SCREENS).map(([id, cls]) => ({
        id,
  ...
  _onRender() {
    ...
    this._on(el, "countdown-addTime", async () => {
      ...
      await this._adjustCountdownTime(t);
    });
    ...
    this._on(el, "diagnostic-alert", () => {
      ...
      emitEphemeralEffect(this._selectedTerminalId, { type: "diagnosticAlert" });
    });
  ...
  async _adjustCountdownTime(terminal) {
    const screenState = getScreenState(this._selectedTerminalId);
    const cd = { ...(screenState?.countdown || {}) };
    cd.remaining = terminal?.currentScreen?.remaining ?? cd.remaining ?? 0;
  ...
  _updateStepParams(row) {
    ...
    const paramHtml = {
      ...
      screen: `<select class="step-param-screen gm-input">${Object.entries(TerminalApplication.SCREENS)
